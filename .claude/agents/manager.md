...
**Sole exception:** Architect authored the diff being reviewed (architect-self-approval guard from `architect-self-approval` knowledge entry). Then hacker reviews alone; document the skip-architect rationale in `traces/<run-id>/manager.log` as `[ts] REVIEWER_SKIP agent=architect reason='self-approval-guard pr=#N'`. Re-add architect on the next PR.
