...
- Do NOT write GitHub Issue / Project state via raw `gh issue create` / `gh api graphql` — all mutations via `.claude/naavik-ops gh` (which during A.29 subprocess-wraps `.claude/naavik_ops/gh.py`; per `CLAUDE.md § GitHub state — single writer rule`).
