...
2. **Manager produces** structured standup:
   ...
   - **Backlog by epic** (post-A.28) — top epic in Backlog by Priority + item count. Use `scripts/gh-project.sh backlog-by-epic --top 3`. Surface deferred work at glance so operator can decide whether to promote.
   ...
   - **Drift** — output of `scripts/gh-project.sh sync` (drift count + diffs). > 0 → recommend `/sync-roadmap --apply` to reconcile (ROADMAP wins). Note: `sync` preserves Backlog (Backlog → Todo is not drift).
