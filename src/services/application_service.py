"""Application service — facade over `services/applications/` (plan 91 4.2).

The former 2216-LOC god-module is decomposed into
`services/applications/{common,state,queries,drafts,submission,auto_apply,
email_suggestions,engagement,export}.py`. This module re-exports every name
(public + the private helpers tests touch) so:

- all route/service importers keep resolving (`application_service.X`),
- the ~60 conftest attribute shims keep landing on the module object every
  caller reads through, and
- the `patch("services.application_service.X")` targets — including the
  `ats_dispatch` alias — keep intercepting, because the submodules route
  their internal cross-seam calls back through this facade (`svc()`).
...
# Patched seam (8 tests patch `services.application_service.ats_dispatch`);
