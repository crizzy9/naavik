...
> Last updated: 2026-05-19 (**4 plans shipped today: 0.2.0.01 vault deprecation + 0.7.0.13 task move fix + 0.2.0.05 Job hardening + 0.2.0.06 Crawl4AI substrate.** Plan 29 / `0.2.0.06` EXECUTED via PR #96 squash `90e2928`. Substrate-only: `src/scraper/{types.py,base.py,crawl4ai_client.py,sites/sample.py}` + `src/services/scraper_service.py` + `docs/design/SCRAPER_BASE.md`. `crawl4ai==0.8.6` + `playwright` promoted to base deps. Architect REQUEST_CHANGES → delta W6 added `RawJob.to_upsert_payload()` adapter (resolved HIGH); 4 hacker findings filed as `0.2.0.06a` (MEDIUM, must ship before `0.2.0.07`). 785 tests passing. Plan 27 / `0.2.0.05` EXECUTED via PR #95 squash `693bb1e` — Job hardened + `JobScrapeRun` + service layer; graduated to `docs/design/JOB_MODEL.md` (470 LOC × 11 sections); 5 hacker findings filed as 0.7.0.15-19 follow-ups; 4 deviations promoted. Plan 28 / `0.7.0.13` EXECUTED via PR #94 squash `70ee0d3` — `task move` gap-only fix + `gh clear-priority` subcommand + 5-doc codification + 12 GH title restores; 3 deviations. Plan 26 / `0.2.0.01` EXECUTED via PR #92 squash `b91f50b` — vault deprecated; env-only secrets; 7 deviations.)
>
> Earlier line: 2026-05-19 (**`0.2.0.01` EXECUTED — PR #92 merged at squash `b91f50b`; closes Issue #20; vault deprecated → env-only secrets.** `src/services/vault.py` (436 LOC AES-256-GCM/PBKDF2/audit) + `src/cli/{vault,init}.py` (258 LOC) + `tests/test_vault.py` DELETED. New `src/services/env_secrets.py` exposes per-scope presence indicators sourced from pydantic-settings. Alembic 0004 drops 5 vault-derived `Settings` columns (`llm_api_key_fingerprint` + 4 `*_configured` bools). `PUT /api/v1/settings/{llm,notifications}` reject `api_key` / `discord_webhook_url` / `telegram_bot_token` / `telegram_chat_id` with 422. Settings UI: API-key input + vault-locked banner gone; env-presence indicator cards instead. `.env.example` rewritten w/ 14 slot rows incl. new `TELEGRAM_CHAT_ID`. `naavik vault <...>` + `naavik init` exit 2 w/ deprecation hint pointing at CHANGELOG. On-disk artifacts retired: `~/.naavik/secrets.enc{,.lock,.bak.*}`, `~/.naavik/key.bin`, `~/.naavik/logs/vault-audit.log`. New regression lint `tests/test_no_vault_imports.py`. CHANGELOG `## [0.2.0]` Unreleased section bootstrapped (Removed/Changed/Added/Operations/Security) w/ loud self-hoster migration paragraph. 8 commits W0-W5 + W6/W7/W8 delta. Reviewers (delta cleared): hacker APPROVE (round-1 1 LOW + 2 INFO defer-recommendations stand); architect APPROVE FULL spec match (round-1 2 MEDIUM + 1 LOW resolved in delta W6+W7). 7 substantive engineer deviations promoted into archived plan; `Skill: engineer-deviation-log` miss noted as lesson candidate. Only surviving CLI subcommand: `naavik serve` (dies in `0.2.0.02`). `0.2.1.03` (Argon2id upgrade DEF-17) auto-closed as moot — vault gone. Next: dispatch `0.2.0.05 HIGH #15` (SQLModel Job + StatusHistory models — gates all scrapers `0.2.0.06`–`0.2.0.10`) OR `0.2.0.04 #70` (PC.6b onboarding bypass fix).)
>
...
| 0.2.0.06a | Scraper hardening — SSRF/LFI scheme allowlist + errors[] redaction + max_listings cap + asyncio fix | [ ] | MEDIUM | A.37 (hacker PR #96) | **Filed 2026-05-19** via PR #96 hacker review (Issue #97). Hacker findings: (1) MEDIUM SSRF/LFI in `Crawl4AIClient.fetch_html` — accepts `file://` URLs; fix scheme allowlist + `pydantic.HttpUrl`; (2) MEDIUM `JobScrapeRun.errors[]` captures URLs+exc without redaction; fix `_safe_url` + `_safe_exc` helpers; (3) LOW `ScrapeQuery.max_listings` no upper bound; fix `Field(le=10_000)`; (4) LOW `asyncio.get_event_loop` deprecated; fix `get_running_loop`. ~35 LOC + 8 tests. **Must ship BEFORE 0.2.0.07** (first real site scrapers wire SSRF surface). |
