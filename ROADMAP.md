...
| 0.7.0.13 | Fix `task move` position-stability + add `gh clear-priority` + codify principle in 5 docs + restore 12 corrupted GH titles | [ ] | MEDIUM | A.31 | **Filed 2026-05-19** — bug surfaced in run `2026-05-19T05-40-56_194aa5` when `task move 0.2.0.02 0.2.1.05` auto-renumbered ~10 sibling tasks in `0.2.0` patch. Principle locked in `.claude/memory/knowledge/patch-version-position-stability.md`. Plan 28 implements: (A) fix `cmd_move` source-section shift to leave gap, dest reject-on-collision; (B) add `gh clear-priority` subcommand; (C) codify in AGENTS.md + docs/design/PHASE_NUMBERING.md + docs/PLAYBOOK.md + .claude/agents/manager.md + .claude/skills/manager-pick-next/SKILL.md; (D) restore 12 corrupted GH titles in W3. `docs/plans/28-fix-task-move-position-stability.md` APPROVED. |
