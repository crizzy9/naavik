...
| PC.6 | Password complexity rules (min 12 chars, must contain digit + letter) + must-change-on-first-login flag for env-injected dev creds | [~] | **Plan 18 IN REVIEW 2026-05-17** (`docs/plans/18-pc6-password-complexity.md`, PR #50 at `feat/PC.6-password-complexity` @ `baad10c3`). Wave-1 implementation shipped: `validate_password_complexity` + `User.must_change_password` (alembic 0003) + `require_password_complete` dep + `POST /api/v1/auth/change-password` + `GET /auth/change-password` page + rotation-unlinks-`~/.naavik/dev-credentials` + cookie/CSRF rotation. PR review surfaced 3 hacker findings + scope-gap (find-replace caught 5 of ~25 mutation surfaces — rest on plan-09 auth stub). Path C re-loop in flight: gate stub `src/ui/routes/settings.py:411` + add `Depends(require_csrf)` to `post_change_password` + AGENT_OPS § 2.8 hook-regex case-sensitivity note + plan 18 deviations section. Broader gate filed as PC.6a (below). |
