...
| A.15 | **Agent memory + learning system** — persistent stores + indexed knowledge base + discussion-capture + periodic learning + session analysis | [ ] | HIGH | **Plan: `docs/plans/19-agent-memory-and-learning.md` (APPROVED 2026-05-17 — all 7 Q decisions locked).** 3-Wave scope shipped in ONE PR (deviation from architect's recommendation per user). **Wave 1** (substrate): `.claude/memory/` + 6 JSONL/md stores (`decisions.jsonl`, `discussions.jsonl`, `lessons.jsonl`, `knowledge/<topic>.md` × 5 seeded, `recurring-patterns.jsonl`, `runs-analysis/<run-id>.md`) + `scripts/agent-memory.sh` (single-writer) + 2 skills (`naavik-memory-lookup` + `naavik-discussion-capture`) + read-only `~/.claude/projects/.../memory/MEMORY.md` integration. **Wave 2** (analytics): `/learn` slash command + `naavik-learn` skill mirror + failure-pattern detection + skill-activation stats + token hotspots + `SKILL_INVOKE` trace event format if needed. **Wave 3** (promotion): recurring-patterns → lessons → knowledge entries at threshold 5; alias mining; threshold tuning hooks. Graduates to `docs/design/AGENT_MEMORY.md` on ship. Branch: `feat/A.15-agent-memory`. Engineer ships everything in one PR; hacker + devops review at PR_REVIEW_GATE. |
