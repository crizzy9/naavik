...
| 0.2.0.11d | Jinja `csrf_token` template-context injection — close HTMX-403 gap on Discover Save/Skip + change-password page | [ ] | HIGH | (reviewers PR #138) | **Filed 2026-05-20 via PR #138 hacker + architect parallel reviews (#139).** Both reviewers independently surfaced same defect: empty `csrf_token` Jinja variable means every HTMX POST sends `X-CSRF-Token: ""` → 403 in production. PRE-EXISTING since plan 18 (change-password page, 2026-05-17); exposed by PR #138 enforcing CSRF on Discover Save/Skip. Tests pass because they thread cookie+header explicitly via TestClient; HTMX UI flow doesn't. **Fix**: Jinja context processor that reads `naavik_csrf` cookie + injects into ctx (OR `templates.env.globals` registration OR `Jinja2Templates.TemplateResponse` override — architect picks at plan time). Scope: ~5-10 LOC + 2 tests (one for /discover swipe, one for /change-password) + Playwright/raw-HTML retest for both surfaces. **HIGH because UX-blocking** — Discover swipe queue + password rotation both 403 in prod until shipped. Memory candidate: "template variable wiring requires evidence-backed risk row" (architect note). |
