...
| **0.2.0** | Job Scraping & Discovery (vault sunset + 6 site scrapers + AI extraction + dedup + scheduler + UI + notifications + rate-limiting + n8n migration) | 🟢 Active | 14/15 shipped 2026-05-20 (PC.6b onboarding bypass closed; only Backlog row 0.2.0.14 + follow-up letter rows remain) |
...
| 0.7.0.27 | `naavik-ops task next-unblocked` returns done tasks — `issue-map.json:statuses` never populated by `gh set-status` | [ ] | MEDIUM | (dogfood discovery 2026-05-20) | **Filed 2026-05-20 via session-handoff verification (#132).** `gh set-status <item-id> Done` updates the Project board (GraphQL mutation) but does NOT write to the local `.claude/github-issue-map.json:statuses` sub-dict — which is what `task next-unblocked` reads to filter blocked-by deps. Result: next-unblocked returns 0.2.0.01 (vault deprecation, DONE) even though ROADMAP marks it [x]. Fix path: either (a) `set-status` writes both Project + local cache, OR (b) `next-unblocked` reads ROADMAP [x] state as source of truth, OR (c) `gh sync` syncs Project Status → local cache. Probably (b) — single source of truth principle; ROADMAP is authoritative anyway. |
