...
Plan 60 / 0.2.7.17 removed the `NAAVIK_PERSISTENCE` env var. Some Phase 1 routes still read fixture data from `src/db/sample_data.py`; the underlying Postgres database is populated by `db/seed.py` at first boot regardless. To verify your data is in Postgres: `psql -h 127.0.0.1 -p 5433 -U naavik -d naavik -c "SELECT headline FROM profile WHERE user_id=1"`. Future plans migrate remaining routes onto the `services/*` layer.
