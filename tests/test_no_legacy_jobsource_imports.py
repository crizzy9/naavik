...
2. `src/ui/discover_ctx.py:build_discover_ctx` must remain wired to
   `services.jobs.list_jobs` (plan 36 cut the umbilical from
...
def test_discover_ctx_wires_job_service_list_jobs():
    ...
    assert "job_service.list_jobs" in body, (
        "src/ui/discover_ctx.py must call services.jobs.list_jobs for the "
    ...
    # combined multi-name form binding `job_service` (used after the
    # plan-69 service-layer expansion), or the long-form `from services.jobs`
    ...
    import_pattern = re.compile(
        ...
        r"^\s*from\s+services\.jobs\s+import\b",
        re.MULTILINE,
    )
