...
- **What:** `Settings.model_config` gained `populate_by_name=True`; `tests/test_config.py` simplified to use `debug=True` / `debug=False` kwargs directly (no more env-var workaround in test 4); `_env_isolated()` strip-list reduced to `("NAAVIK_DEBUG", "SECRET_KEY")` since the `DEBUG` alias was dropped in finding 1. (Commit `8337bb8`.) **Why:** Hacker finding 3 — the original ship's workaround comment ("kwarg `debug=True` is dropped (extra=\"ignore\")") matched § C's original test intent only by accident; setting `populate_by_name=True` makes kwargs flow through cleanly as plan § C originally specified. **Impact:** PC.10 follow-up not needed. Tests now express plan § C's intent unmodified.
