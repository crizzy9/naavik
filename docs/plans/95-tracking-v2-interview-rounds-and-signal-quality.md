...
The 2026-07 redesign made the pipeline _work_ (classification was silently dead
...
1. A process is not one event — Camber ran a technical screen AND a system
   ...
   ("Interview Stage") bucket. The owner often _knows the full round plan_
...
| Company-key canonicalization                                            | `inference._norm` / `processes._norm_company` | Strip legal suffixes (`inc`, `llc`, `ltd`), TLD-ish tails (`.ai`, `.io`, `.com`), collapse spaces/case before grouping and matching. Merges "Brico"/"Brico.ai", "Mosaic"/"mosaicapp.com", "ONO AI"/"Onoai" at the _grouping_ layer, not just the fuzzy matcher. |
...
| C. **First-class `InterviewRound` table** (recommended)          | High — rounds are queryable, link to the emails/calendar events that evidenced them, render as sub-timeline | Medium (one table + one panel + extractor wiring)         | Low — stage machine untouched; rounds are _within_ a stage                                                    | Low         | Low                                     |
...
the contract for board/KPIs (a round never _is_ a stage — it _evidences_ one).

```
InterviewRound
  id, user_id, application_id (FK, CASCADE)
  round_no        int            — display order
  kind            str + CHECK vocabulary (NOT a native Postgres enum — see below):
                    recruiter_screen | technical_screen | take_home |
  ...
  sessions        JSONB []       — clubbed-loop sub-sessions (see below)
...
**Vocabulary grows over time (owner decision, 2026-07-07).** `kind` is a
string column with a CHECK constraint (the 0040 closed-vocab pattern), not a
native enum — extending it is a two-line migration instead of an enum-type
alter, because this list WILL grow as new interview formats appear. Starting
vocabulary is the 10 kinds above; the LLM extractor is told the vocabulary
and instructed to fall back to `other` + verbatim `title` for anything novel
(novel titles accumulating under `other` are the signal to extend the vocab).

**Clubbed onsite loops (owner decision, 2026-07-07).** Companies often bundle
3–5 back-to-back interviews into one "onsite" block delivered as a single
email/invite. That is ONE round of `kind=onsite_loop` whose `sessions` JSONB
holds the itemized sub-sessions —
`[{title, starts_at?, interviewer?, kind_hint?}, …]` — extracted when the
email/calendar invite itemizes them, empty otherwise. One round because the
company treats it as one gate (one outcome, one date); sessions because the
owner still wants to see the segments. The rounds UI renders `onsite_loop`
rows expandable; a later email adding segments upserts into `sessions`
rather than creating sibling rounds.
...
2. **Recruiter notes → process map** (the owner's explicit ask). A "Parse
   ...
   one LLM call extracts the _expected_ rounds → rows created as
...
- Weekly scheduler job (`tracking.staleness_sweep`) flags active applications
  with no signal for **30 days** — one flat threshold for every stage (owner
  decision, 2026-07-07; the earlier two-tier quiet/stale idea and
  stage-sensitive thresholds are dropped for now — revisit only if 30d-flat
  proves noisy). Threshold lives in Settings (`staleness_stale_days`,
  default 30).
...
extracted as the company (TriEdge, Camo People), (b) the _end-client_ is the
...
right, with one refinement:** the _agency_ is never the process, but an agency
...
treatment (agency | ignore | employer), created_from_message_id`. Panel
   ...
   job-related / Actually an employer). Rules are checked _before_ the LLM
...
**Notification posture (owner decision, 2026-07-07):** parked agency mail is
**fully silent** — no Discord/Telegram fan-out, no badge counts; it is
visible only when the owner opens the collapsed group. The owner's default
is "don't track agencies"; the parked group exists purely so nothing is
irrecoverably dropped. Richer agency handling is explicitly deferred until
real need shows up.
...
2. **Few-shot injection**: `classify_email` prompt gains an
   ...
   This is per-user personalization _without training anything_.
   **PII scrub (owner condition, 2026-07-07 — cloud providers OK only if
   exemplars leak no addresses/sensitive data):** exemplars carry the sender
   DOMAIN only (never the full address), the subject, and a snippet run
   through a deterministic scrubber — email addresses, phone numbers, and
   URLs with query strings replaced by placeholders. The scrubber is a pure
   function with its own unit tests; the eval harness (item 3) asserts no
   raw `@`-address survives in any rendered prompt.
...
| C. **Hybrid cascade** (recommended _future_ direction) — rules → correction-kNN via pgvector → LLM only on misses | ≥ A on seen mail (corrections are exact precedent), = A on novel                                     | ~60–80% fewer LLM calls | Fast path ~ms | Mostly local                                                           | Low-medium: embeddings already in the stack (pgvector, `embed` method on providers)                                              |

**Recommendation.** At ~10–40 job-related emails/day the LLM spend is cents
per month — the bottleneck is _label quality on edge cases_, not cost or
...
2. When correction volume justifies it: **C** — embed each incoming
   ...
   reuse that label without an LLM call. The corrections table _is_ the
...
- `client.select("INBOX", readonly=True)` — EXAMINE semantics: the server
  rejects _all_ flag mutations for the session (belt).
...
**Default confirmed (owner decision, 2026-07-07): "To review" stays the
default** — the owner's workflow is paste → let Naavik generate the tailored
resume/cover letter → apply manually with those documents. That path is the
existing queue: To review/Saved → generate docs (`ready_to_submit` when no
auto-submit adapter) → owner applies themselves → flips to Applied (manually
or via the application-receipt email, which now auto-links). "Applied" is one
click away on the same control for jobs being back-filled after the fact.
...
| C. **Provenance-aware precedence** (recommended): manual moves _pin_ the status; email may still _suggest_, and may auto-apply only strictly-forward moves that don't contradict the pin | Keeps automation where it helps, human always wins conflicts | Medium | Low — worst case is one extra confirmation click                                                                                         | Low         |
...
2. **Backward manual moves pin.** If the owner drags a card backward (e.g.
   ...
   email pipeline will not re-apply a transition _to the same status_ the
...
6. **Unpinning is a first-class affordance (owner requirement, 2026-07-07)**
   — the concern: a pinned application would stop auto-tracking for the rest
   of a long process. So the pin must be visible and cheap to lift:
   - a pinned card/slide-over shows a small "auto-tracking paused for
     <status>" chip with a **Resume auto-tracking** action that clears the
     pin;
   - when a suppressed suggestion arrives on a pinned application, the
     suggestion banner offers **"Apply & resume auto-tracking"** alongside
     Apply/Dismiss — accepting the machine's next call is the natural signal
...
with the escape hatch above so "forever" really means "until the human says
otherwise".
...
#### 3.9.1 Full email bodies — the 0.5.0.05a opt-in, folded in (owner decision, 2026-07-07)

The owner wants the full-body opt-in revisited now: a 240-char snippet is
...
excerpt and the classifier prompt uses it instead of the snippet — this is
also the single highest-leverage classification-quality lever in this plan
(most misclassifications trace to the 240-char cap). The privacy contract in
`TRACKING_PIPELINE.md` gets amended from "never bodies at rest" to "snippet
by default; bounded excerpt with explicit per-account opt-in".
...
LinkedIn/Greenhouse scraping as a _different_ Job row. Tier-3 dedup
(`services/jobs/dedup.py`) already catches the pair cross-source (trigram
company 0.6 + role 0.4, threshold 88) — but it only _shadows_ the new row
...
for _different roles_ must not merge (role weight 0.4 + threshold 88 already
...
`interview_round.kind` ships as a string + CHECK vocabulary (extensible via
two-line migrations, per the 0040 pattern) with a `sessions` JSONB for
clubbed onsite loops — both per owner decisions above.

§§ 3.7–3.10 need almost no schema: the manual-URL flow reuses `upsert_job` +
Application creation, the status pin lives in the `submission_artifacts`
JSONB slot, and the enrichment merge writes existing Job columns. The § 3.9.1
body work adds two `email_message` columns (`imap_uid` for on-demand fetch,
`body_excerpt` nullable/opt-in) plus the `email_store_body_excerpt` Settings
toggle — folded into the same `0042_tracking_v2` migration.
...
| 95h   | Manual precedence contract + status pin + unpin affordances (§ 3.8) — small, high-trust win                                 | S                                |
...
canonicalization. 95l is worth pulling forward with 95i: the excerpt is both
the chain's reading surface and the single biggest classification-quality
lever (most residual misclassifications trace to the 240-char snippet cap).
