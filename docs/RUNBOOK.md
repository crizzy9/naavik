...
**Root cause:** `email_sync.sync_account` flipped the account to `AUTH_REQUIRED`, either after an `imaplib.IMAP4.error` (wrong password, revoked app-password, or provider-side throttling) OR because the stored credential could not be decrypted — i.e. `SECRET_KEY` rotated since the password was saved, so the Fernet token no longer decrypts (see Trust posture below). The cron skips accounts in this state until you re-paste the credential.

**Recovery:** Generate a fresh provider app-password (Gmail: Security -> App passwords; Outlook: account.live.com -> Security -> App passwords; Fastmail: Settings -> Privacy & Security -> App Passwords) and re-submit the Connect IMAP form at `/integrations/email`. The route does a one-shot `test_imap_connection` before persisting, so a successful save guarantees the next cron tick will pick up new mail. (If the cause was a `SECRET_KEY` rotation, re-pasting the SAME app-password is enough — it re-encrypts under the new key.)

**Trust posture (DO NOT skip — sets correct operator expectations):** the IMAP password is stored as a **Fernet ciphertext token** in the `email_account.imap_password` Postgres column (plan 90 § A.2.a — OWNER-APPROVED 2026-06-25), not plaintext. The encryption key is `SHA-256(SECRET_KEY)`, so the trust posture is identical to JWT signing: an attacker with `SECRET_KEY` can decrypt both, but a `pg_dump` alone (no `SECRET_KEY`) yields only ciphertext. This is column-level encryption via `services/email_credentials.py`, NOT a vault revival — no `.enc` file, no `key.bin`, no audit log, no CLI (per AGENTS.md § Key Conventions § CLI). Use a provider-issued app-password (never your main account password) and rotate / disable in the provider's UI when you stop using Naavik.

**SECRET_KEY rotation behavior:** if you change `SECRET_KEY`, every previously stored IMAP token becomes undecryptable. `load_imap_password` fails closed (returns `None`, never the raw column value), and the next `tracking.sync_emails` tick flips the account to `AUTH_REQUIRED`. No mail data is lost — only the credential needs re-entry. Re-paste the app-password via the Connect IMAP form to re-encrypt under the new key.
