...
# ===========================================================================
# update-index — regenerate .claude/memory/knowledge/INDEX.md from front-matter
...
_update_index_inline() {
  ...
  {
    echo "# Knowledge Index — \`.claude/memory/knowledge/\`"
    echo ""
    echo "<!-- AUTO-GENERATED by \`scripts/agent-memory.sh update-index\`. Do NOT hand-edit. -->"
    echo "<!-- Maintained on every \`record-knowledge\` invocation. Single-writer rule applies. -->"
    echo ""
    echo "Static index of all knowledge entries. Dynamic index: \`bash scripts/agent-memory.sh list knowledge\`."
    echo ""
    echo "Aliases drive \`Skill: naavik-memory-lookup\` discovery. Add a new entry via:"
    echo "  \`bash scripts/agent-memory.sh record-knowledge <slug> <body-file> --aliases \"a, b\" --confidence H\`"
    echo ""
    echo "| Topic | Confidence | Aliases | First captured | Last referenced |"
    echo "|---|---|---|---|---|"
    for f in "$KNOWLEDGE_DIR"/*.md; do
      ...
      b="$(basename "$f" .md)"
      [[ "$b" == "INDEX" ]] && continue
      local aliases conf first last
      aliases="$(awk -F': ' '/^Aliases:/{print $2; exit}' "$f")"
      conf="$(awk -F': ' '/^Confidence:/{print $2; exit}' "$f")"
      first="$(awk -F': ' '/^First captured:/{print $2; exit}' "$f")"
      last="$(awk -F': ' '/^Last referenced:/{print $2; exit}' "$f")"
      printf "| \`%s\` | %s | %s | %s | %s |\n" "$b" "$conf" "$aliases" "$first" "$last"
      COUNT=$((COUNT + 1))
    done
    echo ""
    echo "_${COUNT} entries. Generated $(now_iso)._"
  } > "$TMP"
  mv "$TMP" "$INDEX"
...
Wave 1 — substrate:
  ...
  update-index                          Regenerate knowledge/INDEX.md from all entries' front-matter.
                                        Auto-fires on record-knowledge; standalone for forced refresh.
